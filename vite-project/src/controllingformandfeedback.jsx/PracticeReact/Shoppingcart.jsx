...
const add =(product)=>{
    let exist = CartItems.find(f=> f.id === product.id)
    if (exist) {
        setCartItems(CartItems.map(m=>(
            m.id === product.id ? {...exist, qty: exist.qty + 1} : m
        )))
    }
    else{
...
const remove = (product) =>{
    let exist = CartItems.find(f=>f.id === product.id)
    if (exist.qty === 1) {
        setCartItems(CartItems.filter(m=> m.id !== product.id )  )
    } else {
